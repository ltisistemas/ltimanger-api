...
export default class CompanyBoardListTaskDaoController {
  ...
  public async index(id = 0, company_list_id = 0, title = '') {
    ...
    if (company_list_id !== 0) params['company_list_id'] = company_list_id
    if (title !== '') params['title'] = title

    return await Database.from(this.tableName).where(params)
  }

  public async counted(id = 0, company_list_id = 0, title = '') {
    const params = {}
